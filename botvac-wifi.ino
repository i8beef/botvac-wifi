...
void update() {
  if (WiFi.status() == WL_CONNECTED) {
      ...
      if (errorInfo == "" || errorInfo == "-FAIL-" || lastErrRun > 11) {
        getError();
        lastErrRun = 0;
      } else {
        lastErrRun++;
...
void getError() {
  ...
  String errorInfoTemp = Serial.readString();
  // Check for dash in error
  int serialString = errorInfoTemp.indexOf(" - ");
  int capUntil = serialString-4;

  if (serialString > -1){
    errorInfo = errorInfoTemp.substring(capUntil, serialString);
  } else {
    errorInfo = "None";
  }
}
...
TimedAction updateTimer = TimedAction(1000, update);
...
void consoleRequestHandler() {
  ...
  "var b='ws://'+location.hostname+':81/',c,d,e;function g(){d=new WebSocket(b);d.onopen=function(){h('[connected]')};d.onclose=function(){h('[disconnected]')};d.onmessage=function(a){h('<span style=\"color: blue;\">[response] '+a.data+'</span>')};d.onerror=function(a){h('<span style=\"color: red;\">[error] </span> '+a.data)}}\nfunction k(a){if(13==a.keyCode){a=e.value;if('/disconnect'==a)d.close();else if('/clear'==a)for(;c.firstChild;)c.removeChild(c.firstChild);else''!=a&&(h('[sent] '+a),d.send(a));e.value='';e.focus()}}function h(a){var f=document.createElement('p');f.innerHTML=a;c.appendChild(f);window.scrollTo(0,document.body.scrollHeight)}\nwindow.addEventListener('load',function(){c=document.getElementById('c');e=document.getElementById('i');g();document.getElementById('i').addEventListener('keyup',k,!1);e.focus()},!1);" +
  "</script>" +
  "</head>" +
  "<body>" +
  "<h2>Neato Console</h2>" +
...
void lidarRequestHandler() {
  server.send(200, "application/json", "{ lidarInfo: \"" + lidarInfo + "\" }");
}

void batteryInfoRequestHandler() {
  server.send(200, "application/json", "{ batteryInfo: \"" + batteryInfo + "\" }");
}

void errorInfoRequestHandler() {
  server.send(200, "application/json", "{ errorInfo: \"" + errorInfo + "\" }");
}

void cleanRequestHandler() {
...
void setup() {
  ...
  // start webserver
  server.on("/console", consoleRequestHandler);
  ...
  server.on("/clean", HTTP_POST, cleanRequestHandler);
  server.on("/lidar", HTTP_GET, lidarRequestHandler);
  server.on("/battery", HTTP_GET, batteryInfoRequestHandler);
  server.on("/error", HTTP_GET, errorInfoRequestHandler);

  server.onNotFound(consoleRequestHandler);
  server.begin();
